...
@elseif($totalPage >= 2)
    @if($page==1) <li class="disabled"><span>上一页</span></li>
    	@else <li><a href="?{{$page_query}}&page={{$page-1}}">上一页</a></li>
    @endif
    @if($totalPage-$page > 5)
            @if($page > 6)
                <li><a href="?{{$page_query}}&page=1">1</a></li>
                <li><span>...</span> </li>
                @for($index=$page-5;$index<$page+4;$index++)
                    @if($index+1 == $page)
                       <li  class="active"> <span>{{$page}}</span> </li>
                    @else
                         <li><a href="?{{$page_query}}&page={{$index+1}}">{{$index+1}}</a> </li>
                    @endif
                 @endfor
            @else
            ...
            @if($totalPage >10)
            <li><span>...</span> </li>
           <li> <a href="?{{$page_query}}&page={{$totalPage}}">{{$totalPage}}</a> </li>
            @endif
    @else
    ...
    @else <li><a href="?{{$page_query}}&page={{$page+1}}">下一页</a> </li>
    @endif
@endif
</ul>
