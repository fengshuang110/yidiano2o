...
class Helper
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".Pay_Wechat_Config::KEY;
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, Pay_Wechat_Config::SSLCERT_PATH);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, Pay_Wechat_Config::SSLKEY_PATH);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
 ...
 * 请求型接口的基类
 */
class Wxpay_client_pub extends Helper 
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = Pay_Wechat_Config::APPID;//公众账号ID
	   	$this->parameters["mch_id"] = Pay_Wechat_Config::MCHID;//商户号
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = Pay_Wechat_Config::CURL_TIMEOUT;
	}
 ...
 * 响应型接口基类
 */
class Wxpay_server_pub extends Helper 
{
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = Pay_Wechat_Config::APPID;//公众账号ID
		   	$this->returnParameters["mch_id"] = Pay_Wechat_Config::MCHID;//商户号
 ...
 * 静态链接二维码
 */
class NativeLink_pub  extends Helper
{
...
* JSAPI支付——H5网页端调起支付接口
*/
class JsApi_pub extends Helper
{
	...
	{
		$urlObj["appid"] = Pay_Wechat_Config::APPID;
	...
	{
		$urlObj["appid"] = Pay_Wechat_Config::APPID;
		$urlObj["secret"] = Pay_Wechat_Config::APPSECRET;
	...
	{
		$jsApiObj["appId"] = Pay_Wechat_Config::APPID;
		$timeStamp = time();
