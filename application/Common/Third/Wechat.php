...
class Wechat
{
	...
	{
		if(empty($options)){
			$config=  require(__DIR__.'/Conf/weixin.php');
			if(empty($config)){
				throw new \Exception('can not found the weixin_config  in Third/Conf/account.php');
			};
	...
	public function getAccess_token(){
		$memcache = Cache::getCacheAdapter();//定义缓存实例
		$access_token = $memcache->get($this->appid."_access_token");
		if($access_token){
			...
			if ($result)
			{
				...
				$memcache->set($this->appid."_access_token", $this->access_token,$expire);
				return $this->access_token;
			}
			return false;
		}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		$memcache = Cache::getCacheAdapter();
		
		$access_token = $memcache->get($appid."_access_token");
		...
		if($access_token){
			...
			if ($result)
			{
				...
				$this->access_token = $json['access_token'];
				$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
				$memcache->set($appid."_access_token", $this->access_token,$expire);
				return $this->access_token;
			}
			return false;
		}
	...
	public function getSignPackage($http_host="") {
		$jsapiTicket = $this->getJsApiTicket();
	
		// 注意 URL 一定要动态获取，不能 hardcode.
		$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
// 		$http_host = str_replace($protocol, "", $http_host);
		if($http_host){
			$url=$http_host;
		}else{
			$url = "$protocol"."m.bqmart.cn"."$_SERVER[REQUEST_URI]";
		}
	...
	private function getJsApiTicket() {
		...
		$ticket = $memcache->get($this->appid."_jsapi_access_token");
		...
		if (!$ticket) {
			...
			// $url = "https://qyapi.weixin.qq.com/cgi-bin/get_jsapi_ticket?access_token=$accessToken";
				...
				$ticket = $res['ticket'];
				if ($ticket) {
					$memcache->set($this->appid."_jsapi_access_token", $ticket,$res['expires_in']-200);
				}
			}
